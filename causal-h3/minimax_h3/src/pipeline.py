...
def randn_tensor(shape, generator=None, dtype=None, device=None):
    """Local replacement for diffusers.utils.torch_utils.randn_tensor."""
    if generator is not None:
        gen_device = generator.device if hasattr(generator, "device") else None
        latents = torch.randn(shape, generator=generator, device=gen_device or device, dtype=dtype)
    else:
        ...
        **kwargs,
        ...
        for i in range(num_steps):
            ...
            video_pred, audio_pred = transformer(
                ...
                use_causal_mask=model.causal_mode,
