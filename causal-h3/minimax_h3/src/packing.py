...
def build_packed_sequence(
    ...
    media_origin = float(num_text) + media_advance + chunk_offset
...
def compute_chunk_boundaries(
    ...
    """Return a per-row chunk index tensor ``(S,)`` long.

    Each target row is assigned a temporal chunk index based on its
    t-coordinate (the first axis of ``position_ids``).  Non-target rows
    (text, conditioning, pad) get -1.

    A *chunk* corresponds to ``LATENTS_PER_CHUNK`` (5) latent video frames.
    The temporal grid advances by ``_ROPE_FRAME_RESCALE *
    _ROPE_FRAMES_PER_LATENT[i]`` per latent frame (pattern ``(1, 4, 4, 4,
    4)``), so one chunk spans ``_ROPE_FRAME_RESCALE *
    sum(_ROPE_FRAMES_PER_LATENT)`` = ``(5/3) * 17`` = ``85/3`` t-units.
    The t-coordinate is normalised to the target segment's temporal origin
    (the first target row's t) before quantising so that bucket edges align
    with actual chunk boundaries regardless of prompt length or reference
    advance.

    Using actual chunk ids (not sequential indices) lets audio and video
    rows that share the same temporal chunk receive the same id even though
    they occupy separate segments in the packed ``[text | conditions |
    target audio | target video]`` layout.

    Only target rows (video/audio inside the target segment, i.e. not text,
    not keyframe/ref conditioning, not pad) are assigned to chunks.
    Conditioning rows (keyframe anchors or ref2va references) precede target
    rows in the packed layout; *num_condition_rows* gives their count so
    they can be excluded.  Without this exclusion a last-frame keyframe
    anchor lands in a late chunk and becomes invisible to earlier target
    chunks under the causal mask.

    Parameters
    ----------
    token_tags : (S,) long
        Per-row modality tags.
    position_ids : (S, 3) float
        Rotary coordinates (t, h, w).
    num_condition_rows : int
        Number of conditioning rows (video + audio) at the start of the
        non-text, non-pad segment.  These are excluded from chunk assignment
        so they remain visible to all chunks via ``build_causal_mask``.

    Returns
    -------
    torch.Tensor (S,) long
        Per-row chunk index.  -1 for non-target rows.  Empty (all -1) if
        there are no target rows.
    """
    ...
    S = token_tags.shape[0]
    chunk_idx = torch.full((S,), -1, dtype=torch.long, device=token_tags.device)

    is_text = token_tags == TEXT_TAG
    is_pad = token_tags == PAD_TAG

    # target rows = non-text, non-pad, excluding conditioning rows that
    # precede the target segment in the packed layout
    is_target = ~is_text & ~is_pad
    if num_condition_rows > 0:
        nonzero_idx = torch.nonzero(is_target, as_tuple=False).squeeze(-1)
        is_target = torch.zeros_like(is_target)
        is_target[nonzero_idx[num_condition_rows:]] = True

    if not bool(is_target.any()):
        return chunk_idx

    target_indices = torch.nonzero(is_target, as_tuple=False).squeeze(-1)
    t_coords = position_ids[target_indices, 0].to(torch.float64)

    # Normalise to the target segment's temporal origin so bucket edges
    # align with actual chunk boundaries regardless of prompt length.
    t_coords = t_coords - t_coords[0]

    # One chunk per LATENTS_PER_CHUNK latent frames.  The temporal grid
    # advances by _ROPE_FRAME_RESCALE * _ROPE_FRAMES_PER_LATENT[i] per
    # latent frame (pattern (1,4,4,4,4)), so 5 latent frames span
    # _ROPE_FRAME_RESCALE * sum(_ROPE_FRAMES_PER_LATENT) = (5/3)*17 = 85/3.
    chunk_span = _ROPE_FRAME_RESCALE * sum(_ROPE_FRAMES_PER_LATENT)
    chunk_ids = torch.floor(t_coords / chunk_span).long()

    chunk_idx[target_indices] = chunk_ids
    return chunk_idx
